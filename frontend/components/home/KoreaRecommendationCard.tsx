...
import {
    createKoreaRecommendedPlan,
    type KoreaRecommendedCity,
    type KoreaTripDuration,
} from "@/lib/koreaRecommendedPlans";
import { generatePlanId, saveTravelPlan } from "@/lib/travelPlans";
import { useAuthStore } from "@/stores/authStore";

type KoreaRecommendationCardProps = {
...
export default function KoreaRecommendationCard({
    city,
    title,
    description,
    image,
    alt,
}: KoreaRecommendationCardProps) {
    const router = useRouter();
    const { isLoggedIn, me } = useAuthStore();
    const [expanded, setExpanded] = useState(false);
    const [submitting, setSubmitting] = useState<KoreaTripDuration | null>(null);
    const [error, setError] = useState("");

    const expandCard = () => {
        window.dispatchEvent(new CustomEvent("home:focus-city", { detail: { city } }));
        setExpanded(true);
    };
    ...
    const createPlan = async (duration: KoreaTripDuration) => {
        if (isLoggedIn === false) {
            router.push("/login");
            return;
        }
        ...
        try {
            ...
            const plan = createKoreaRecommendedPlan(
                id,
                city,
                duration,
                me?.email
    ...
    return (
        ...
        >
            ...
            >
                ...
                <p className="mt-1 text-sm text-white/85">{expanded ? "추천 일정표를 바로 만들기" : description}</p>
