...
export default function DayCard(props: {
    ...
    return (
        <div className="bg-white border-2 border-gray-200 rounded-xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 hover:border-gray-300">
            {/* Day Header */}
            <div className="bg-gradient-to-r from-gray-900 to-gray-800 px-5 py-4 flex items-center gap-3 group relative overflow-hidden">
            ...
            <div className="p-5 space-y-3 bg-gradient-to-b from-white to-gray-50">
                {day.activities.map((activity, idx) => (
                    <ActivityRow
                        ...
                        onChangeField={(field, value) => props.onUpdateActivityField(day.id, activity.id, field, value)}
                ...
                {day.activities.length > 0 && (
                    <div className="text-right pt-3 mt-3 border-t-2 border-gray-200 bg-gradient-to-r from-transparent to-gray-100 -mx-5 px-5 py-3">
                        <span className="text-sm text-gray-600 font-medium">Day {dayIndex + 1} 총 경비: </span>
                        <span className="font-bold text-xl text-gray-900 ml-2">{dayCost.toLocaleString()}</span>
